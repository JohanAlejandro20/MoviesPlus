...
export class MovieService {
    ...
    orderMovieByParam(param: string): void {
        if (param == "title") {
        ...
        this.movies.sort((a: MovieModel, b: MovieModel) => (Math.random() - 0.5));

    }

    saveMovieWatchList(movie: MovieModel): void {
        let watchList: MovieModel[] = this.getAllMoviesWatchList();
        let validWatchList: boolean = watchList.some(movieWatchList => movieWatchList.id == movie.id)
        if (validWatchList) return;
        watchList.push(movie);
        localStorage.setItem("movieWatchList", JSON.stringify(watchList));
    }

    deleteMovieWatchList(movie: MovieModel): void {
        let watchList: MovieModel[] = this.getAllMoviesWatchList();
        watchList = watchList.filter(movieWatchList => movieWatchList.id != movie.id)
        localStorage.setItem("movieWatchList", JSON.stringify(watchList));
    }
